...
def vbb_anno2dict(vbb_file, cam_id):
    ...
    # person index
    ignore_index_list = np.where(np.array(objLbl) == "ignore")[0]
    count = 0
    ...
    for frame_id, obj in enumerate(objLists):
        if len(obj) > 0 and obj != []:
            ...
            for id, pos in zip(obj['id'][0], obj['pos'][0]):
                ...
                if not id in ignore_index_list:  # only use bbox whose label is person
                    annos[frame_name]["difficulty"].append('0')
                else:
                    annos[frame_name]["difficulty"].append('1')
...
def instance2xml_base(anno, img_size, bbox_type='xyxy'):
    ...
    anno_tree = E.annotation(
        E.folder('KFTEC_VocTyp/person'),
        E.filename(anno['id']),
        E.source(
            E.database('kftec pedestrian'),
            E.annotation('kftec pedestrian'),
            E.image('kftec pedestrian'),
            E.url('None')
        ),
        E.size(
            E.width(img_size[0]),
    ...
    for index, bbox in enumerate(anno['bbox']):
        ...
        anno_tree.append(
            ...
            E.difficult(0),
            E.occlusion(anno["difficulty"][index])
            )
        )
    return anno_tree
...
def main():
    seq_inputdir = "/home/JunYS/data/KFTEC_DATA/"
    vbb_inputdir = "/home/JunYS/data/KFTEC_DATA/annotations"
    seq_outputdir = "/home/JunYS/data/KFTEC_DATA/VOC_type"
    vbb_outputdir = "/home/JunYS/data/KFTEC_DATA/VOC_type"
    parse_anno_file(vbb_inputdir, seq_inputdir, vbb_outputdir, seq_outputdir)
    # xml_file = "/home/JunYS/data/KFTEC_DATA/VOC_type/annotations/set00/bbox/set00_V013_1511.xml"
    # img_file = "/home/JunYS/data/KFTEC_DATA/VOC_type/set00/frame/set00_V013_1511.jpg"
